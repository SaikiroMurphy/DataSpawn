...
path = r"E:\OneDrive - bkacad.edu.vn\Documents\PhieuTN"

count = 5001
for q in tqdm(range(0,500)):
    for file in os.listdir(os.path.join(path, "Storage")):
        if file.endswith(".jpg") or file.endswith(".png"):

            rd = cv2.imread(os.path.join(path, "Storage", file),cv2.IMREAD_COLOR)
            ...
            rd = cv2.GaussianBlur(rd,(3,3), 0)

            copy = rd.copy()

            with (open(os.path.join(path, "Storage", file[:-4] + '.txt')) as conf):
